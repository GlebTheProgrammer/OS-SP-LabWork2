...
CONST INT numOfRows = 3;                    // количество строк в итоговой таблице
    ...
    int nCmdShow)            // число, показывающее, как будет развёрнуто окно
{
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_TEXTINSIDETHETABLE));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDC_TEXTINSIDETHETABLE));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DESTROY: // при выходе из приложения
    {
        PostQuitMessage(0); // посылаем сообщение на выход
    } break;

    default:
        return DefWindowProc(hWnd, message, wParam, lParam); // стандартная процедура обработки
    }
    return 0;
}
...
VOID ArrayInitialization(HANDLE hFile)
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        srand(time(0));
        DWORD numOfBytesToRead = rand() % 254; // Число от 0 до 32767 % 254
...
VOID DrawTable(HWND hWnd, RECT wndRect)
{
    ...
    COLORREF colorText = RGB(255, 255, 255), // цвет текста
        colorBack = RGB(30, 30, 30),   // цвет заднего фона
        colorLine = RGB(128, 128, 128);  // цвет линий таблицы
